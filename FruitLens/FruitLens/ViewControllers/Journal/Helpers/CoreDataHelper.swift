...
class CoreDataHelper {
    ...
    static func readNotesFromCoreData(fromManagedObjectContext: NSManagedObjectContext) -> [JournalEntry] {

        var returnedNotes = [JournalEntry]()
        ...
        do {
            ...
            fetchedNotesFromCoreData.forEach { (fetchRequestResult) in
                ...
                returnedNotes.append(JournalEntry.init(
    ...
    static func readNoteFromCoreData(
        noteIdToBeRead: UUID,
        fromManagedObjectContext: NSManagedObjectContext) -> JournalEntry? {
